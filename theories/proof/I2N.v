...
From Stdlib Require Import ZArith NArith Uint63 Lia ZifyUint63 ZifyN List ZifyNat.
...
Section I2N.
  ...
  Proof.
    apply N2Z.inj. unfold toN. rewrite sub_spec, Z2N.id, N2Z_msub by lia. lia.
  Qed.
  Lemma inj_mul: toN (i * j) = %(n * m). Proof. lia. Qed.
  Lemma inj_mod: toN (i mod j) = (n mod m)%N.
  Proof. unfold toN. now rewrite mod_spec, Z2N.inj_mod by lia. Qed.

  Lemma inj_lsl: toN (i << j) = %(N.shiftl n m).
  Proof.
    unfold toN. now rewrite lsl_spec, N.shiftl_mul_pow2, Z2N.inj_mod, Z2N.inj_mul, Z2N.inj_pow by lia.
  Qed.
  Lemma inj_lsr: toN (i >> j) = N.shiftr n m.
  Proof.
    unfold toN. now rewrite lsr_spec, <-Z.shiftr_div_pow2, Z2N_inj_shiftr by lia.
  Qed.
  Lemma inj_land: toN (i land j) = N.land n m.
  Proof. unfold toN. now rewrite land_spec', Z2N_inj_land by lia. Qed.
  Lemma inj_lor: toN (i lor j) = N.lor n m.
  Proof. unfold toN. now rewrite lor_spec', Z2N_inj_lor by lia. Qed.
  Lemma inj_lxor: toN (i lxor j) = N.lxor n m.
  Proof. unfold toN. now rewrite lxor_spec', Z2N_inj_lxor by lia. Qed.
  ...
  Proof. unfold toN. now rewrite compare_spec, Z2N.inj_compare by lia. Qed.
  Lemma inj_succ: toN (succ i) = %(N.succ n). Proof. lia. Qed.
  Lemma inj_pred: toN (pred i) = msub 63 n 1.
  Proof. unfold toN. apply N2Z.inj. rewrite pred_spec, Z2N.id, N2Z_msub by lia. lia. Qed.
  Lemma inj_min: toN (min i j) = N.min n m.
  Proof. unfold toN. rewrite min_spec. lia. Qed.
  Lemma inj_max: toN (max i j) = N.max n m.
  Proof. unfold toN. rewrite max_spec. lia. Qed.
  Lemma inj_lt: i <? j = true <-> n < m. Proof. lia. Qed.
  Lemma inj_le: i <=? j = true <-> n <= m. Proof. lia. Qed.
End I2N.
Lemma to_of_N : forall n, toN (ofN n) = %n.
Proof.
  intro. unfold toN, ofN. rewrite of_Z_spec, Z2N.inj_mod, N2Z.id; lia.
Qed.
...
Section I2Nat.
  ...
  Lemma id: ofNat n = i. Proof. lia. Qed.
  Lemma inj: n = m -> i = j. Proof. lia. Qed.
  Lemma inj_iff: n = m <-> i = j. Proof. lia. Qed.
  Lemma inj_eqb: (n =? m) = (i =? j)%uint63. Proof. lia. Qed.

  Lemma inj_add: toNat (i + j) = %(n + m). Proof. lia. Qed.
  Lemma inj_mul: toNat (i * j) = %(n * m). Proof. lia. Qed.
  Lemma inj_mod: toNat (i mod j) = (n mod m). Proof. lia. Qed.

  Lemma inj_lsl: toNat (i << j) = %(Nat.shiftl n m).
  Proof.
    unfold toNat. now rewrite lsl_spec, Nat.shiftl_mul_pow2, Z2Nat.inj_mod, Z2Nat.inj_mul, Z2Nat.inj_pow by lia.
  Qed.
  Lemma inj_lsr: toNat (i >> j) = Nat.shiftr n m.
  Proof.
    unfold toNat. now rewrite lsr_spec, Nat.shiftr_div_pow2, Z2Nat.inj_div, Z2Nat.inj_pow by lia.
  Qed.
  Lemma inj_compare: compare i j = Nat.compare n m.
  Proof. unfold toNat. now rewrite compare_spec, Z2Nat.inj_compare by lia. Qed.
  Lemma inj_succ: toNat (succ i) = %(S n). Proof. lia. Qed.
End I2Nat.
End I2Nat.
...
Ltac nify :=
  ...
  | |- @eq int _ _ => apply I2N.inj
  | |- ?i <? ?j = true => apply I2N.inj_lt
  | |- ?i <=? ?j = true => apply I2N.inj_le
  | |- context[toN (?i + ?j)] => rewrite (I2N.inj_add i j)
  | |- context[toN (?i - ?j)] => rewrite (I2N.inj_sub i j)
  | |- context[toN (?i * ?j)] => rewrite (I2N.inj_mul i j)
  | |- context[toN (?i mod ?j)] => rewrite (I2N.inj_mod i j)
  | |- context[toN (?i << ?j)] => rewrite (I2N.inj_lsl i j)
  | |- context[toN (?i >> ?j)] => rewrite (I2N.inj_lsr i j)
  | |- context[toN (?i land ?j)] => rewrite (I2N.inj_land i j)
  | |- context[toN (?i lor ?j)] => rewrite (I2N.inj_lor i j)
  | |- context[toN (?i lxor ?j)] => rewrite (I2N.inj_lxor i j)
  | |- context[bit ?i ?j] => rewrite (I2N.inj_bit i j)
  | |- context[compare ?i ?j] => rewrite (I2N.inj_compare i j)
  end.
Ltac izify :=
  repeat match goal with
